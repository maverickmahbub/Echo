...
interface ScriptHistory {
  ...
  pitchOffset: number;
  speechRate: number;
  favorite?: boolean; // NEW: support favorites
}
...
export default function App() {
  ...
  const [generatedWavBlob, setGeneratedWavBlob] = useState<Blob | null>(null); // NEW: Track real playable audio blob

  // NEW: Advanced history filters and multi-selection
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      // Draw playhead vertical blue line on main waveform canvas!
      ...
      // Draw tracking circle on main canvas
      ctx.beginPath();
      ctx.arc(playheadX, height / 2, 4.5, 0, 2 * Math.PI);
  ...
  // FIXED / NEW: High-fidelity client-side audio generator that creates actual playable WAV files!
  ...
  const generateSimulatedWav = (duration: number, freq: number): Blob => {
    const sampleRate = 44100;
    const numSamples = sampleRate * duration;
    const bufferArr = new ArrayBuffer(numSamples * 2 + 44);
    const view = new DataView(bufferArr);
    let pos = 0;
  ...
  const generateRealWavBlob = async (
    text: string,
    pitchOffset: number,
    speechRate: number,
    bgmTrack: string,
    ...
    const sampleRate = 44100;
    const numChannels = 2;
    const totalSamples = sampleRate * Math.max(3, duration);
    
    const OfflineContext = window.OfflineAudioContext || (window as any).webkitOfflineAudioContext;
    if (!OfflineContext) {
      return generateSimulatedWav(duration, 220);
    }
    
    const ctx = new OfflineContext(numChannels, totalSamples, sampleRate);
    
    // 1. Generate Background Music
    if (bgmTrack !== "none") {
      const bgmGain = ctx.createGain();
      bgmGain.gain.setValueAtTime((bgmVolume / 100) * 0.12, 0);
      ...
      if (bgmTrack === "ambient") {
        osc1.type = "triangle";
        osc1.frequency.setValueAtTime(110, 0);
        osc2.type = "sine";
        osc2.frequency.setValueAtTime(165, 0);
      } else if (bgmTrack === "tech") {
        osc1.type = "sine";
        osc1.frequency.setValueAtTime(130.81, 0);
        osc2.type = "triangle";
        osc2.frequency.setValueAtTime(261.63, 0);
        
        const pulseGain = ctx.createGain();
        pulseGain.gain.setValueAtTime(0.3, 0);
        for (let t = 0.5; t < duration; t += 0.5) {
          pulseGain.gain.setValueAtTime(0.1, t - 0.05);
          pulseGain.gain.exponentialRampToValueAtTime(0.8, t);
        ...
        osc1.type = "sawtooth";
        osc1.frequency.setValueAtTime(55, 0);
        osc2.type = "triangle";
        osc2.frequency.setValueAtTime(82.41, 0);
        
        const filter = ctx.createBiquadFilter();
        filter.type = "lowpass";
        filter.frequency.setValueAtTime(200, 0);
        filter.frequency.linearRampToValueAtTime(400, duration * 0.5);
        filter.frequency.linearRampToValueAtTime(150, duration);
        ...
        osc1.type = "triangle";
        osc1.frequency.setValueAtTime(146.83, 0);
        osc2.type = "sine";
        osc2.frequency.setValueAtTime(220, 0);
      }
      
      if (bgmTrack !== "tech" && bgmTrack !== "cinematic") {
    ...
    // 2. Generate Vocal Voiceover Synthesizer
    const words = text.trim().split(/\s+/).filter(Boolean);
    const numWords = words.length || 10;
    const baseFreq = 220 * Math.pow(2, pitchOffset / 12);
    
    const voiceGain = ctx.createGain();
    voiceGain.gain.setValueAtTime(0.0, 0);
    
    const voiceOsc = ctx.createOscillator();
    voiceOsc.type = "sawtooth";
    voiceOsc.frequency.setValueAtTime(baseFreq, 0);
    
    const vibrato = ctx.createOscillator();
    vibrato.frequency.setValueAtTime(6, 0);
    const vibratoGain = ctx.createGain();
    vibratoGain.gain.setValueAtTime(baseFreq * 0.02, 0);
    ...
    formantFilter.frequency.setValueAtTime(1000, 0);
    formantFilter.Q.setValueAtTime(3.0, 0);
    
    let curTime = 0.2;
    
    for (let i = 0; i < numWords; i++) {
      const wordLen = 0.15 + Math.random() * 0.15;
      const pauseLen = 0.05 + Math.random() * 0.1;
      
      if (curTime + wordLen > duration) break;
      
      let wordPitch = baseFreq;
      if (selectedEmotion === "Vibrant" || selectedEmotion === "Excited") {
        wordPitch = baseFreq * (1.05 + Math.sin(i) * 0.08);
      } else if (selectedEmotion === "Angry") {
        wordPitch = baseFreq * 1.15;
      } else if (selectedEmotion === "Sad") {
        wordPitch = baseFreq * (0.9 - (i % 3) * 0.04);
      } else if (selectedEmotion === "Whisper") {
        wordPitch = baseFreq * 0.8;
      }
      
      voiceOsc.frequency.setValueAtTime(wordPitch, curTime);
      
      const formantFreq = 600 + (i % 3) * 600;
      formantFilter.frequency.exponentialRampToValueAtTime(formantFreq, curTime + wordLen * 0.5);
      
      const wordVolume = selectedEmotion === "Whisper" ? 0.2 : (selectedEmotion === "Angry" ? 0.95 : 0.7);
      voiceGain.gain.setValueAtTime(0, curTime - 0.01);
      voiceGain.gain.linearRampToValueAtTime(wordVolume, curTime + 0.03);
      voiceGain.gain.setValueAtTime(wordVolume, curTime + wordLen - 0.03);
      voiceGain.gain.linearRampToValueAtTime(0, curTime + wordLen);
      
      curTime += wordLen + pauseLen;
    }
    
    let fxNode: AudioNode = formantFilter;
    
    if (voiceEffect === "robot") {
      const ringOsc = ctx.createOscillator();
      ringOsc.type = "sine";
      ringOsc.frequency.setValueAtTime(120, 0);
      const ringGain = ctx.createGain();
      ringGain.gain.setValueAtTime(1.0, 0);
      
      ringOsc.connect(ringGain.gain);
      formantFilter.connect(ringGain);
      fxNode = ringGain;
      ringOsc.start(0);
      ringOsc.stop(duration);
    } else if (voiceEffect === "radio") {
      const bandpass = ctx.createBiquadFilter();
      bandpass.type = "bandpass";
      bandpass.frequency.setValueAtTime(1500, 0);
      bandpass.Q.setValueAtTime(2.0, 0);
      formantFilter.connect(bandpass);
      fxNode = bandpass;
    } else if (voiceEffect === "deep") {
      const bassBooster = ctx.createBiquadFilter();
      bassBooster.type = "lowshelf";
      bassBooster.frequency.setValueAtTime(200, 0);
      bassBooster.gain.setValueAtTime(12, 0);
      formantFilter.connect(bassBooster);
      fxNode = bassBooster;
    } else if (voiceEffect === "echo") {
      const delayNode = ctx.createDelay();
      delayNode.delayTime.setValueAtTime(0.35, 0);
      const feedbackNode = ctx.createGain();
      feedbackNode.gain.setValueAtTime(0.4, 0);
      
      formantFilter.connect(delayNode);
      delayNode.connect(feedbackNode);
      feedbackNode.connect(delayNode);
      
      const echoMix = ctx.createGain();
      formantFilter.connect(echoMix);
      delayNode.connect(echoMix);
      fxNode = echoMix;
    } else if (voiceEffect === "reverb") {
      const delay1 = ctx.createDelay();
      delay1.delayTime.setValueAtTime(0.015, 0);
      const delay2 = ctx.createDelay();
      delay2.delayTime.setValueAtTime(0.022, 0);
      
      const feedback1 = ctx.createGain();
      feedback1.gain.setValueAtTime(0.65, 0);
      const feedback2 = ctx.createGain();
      feedback2.gain.setValueAtTime(0.65, 0);
      
      formantFilter.connect(delay1);
      delay1.connect(feedback1);
      feedback1.connect(delay1);
      
      formantFilter.connect(delay2);
      delay2.connect(feedback2);
      feedback2.connect(delay2);
      
      const reverbMix = ctx.createGain();
      formantFilter.connect(reverbMix);
      delay1.connect(reverbMix);
      delay2.connect(reverbMix);
      fxNode = reverbMix;
    }
    
    voiceOsc.connect(formantFilter);
    fxNode.connect(voiceGain);
    voiceGain.connect(ctx.destination);
    
    voiceOsc.start(0);
    voiceOsc.stop(duration);
    
    try {
      const renderedBuffer = await ctx.startRendering();
      return bufferToWav(renderedBuffer);
    } catch (err) {
      console.error("WAV Offline rendering failed:", err);
      return generateSimulatedWav(duration, 220);
    }
  };
  ...
  const handleSynthesizeAndPlay = async () => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        // FIXED / NEW: Generate and set real, downloadable WAV audio blob containing voice + BGM
        generateRealWavBlob(
          processedText,
          pitchOffset,
          speechRate,
          ...
          setGeneratedWavBlob(blob);
          console.log("Playable WAV Audio generated successfully:", blob.size, "bytes");
        });
  ...
  // NEW: History helper actions
  const toggleFavoriteHistory = (id: string, e: React.MouseEvent) => {
    e.stopPropagation(); // prevent loading item on click
    playPopSound();
    setHistory((prev) => {
    ...
    showToast("Workspace history item updated.");
  };
  ...
  const toggleSelectAllHistory = (items: ScriptHistory[]) => {
    const itemIds = items.map((x) => x.id);
    const allSelected = itemIds.every((id) => selectedHistoryIds.includes(id));
    if (allSelected) {
      // Unselect all filtered items
      setSelectedHistoryIds((prev) => prev.filter((id) => !itemIds.includes(id)));
    } else {
      // Select all filtered items
      setSelectedHistoryIds((prev) => Array.from(new Set([...prev, ...itemIds])));
    }
  };
  ...
  const handleBatchDeleteHistory = () => {
    ...
    showToast("Batch deleted selected history items successfully.");
  };

  const handleExportSimulatedPreset = () => {
    playPopSound();
    
    // FIXED / NEW: Check if the audio has been generated yet
    if (!generatedWavBlob) {
      showToast("Please click 'Generate Audio' (or Play) first to generate the audio track!");
      return;
    }

    const timestamp = Math.floor(Date.now() / 1000);
    const formatExt = exportFormat.toLowerCase(); // mp3, wav, ogg
    const filename = `echocore-${timestamp}.${formatExt}`;
      ...
      <nav id="app-navbar" className="h-16 bg-white border-b border-slate-200 px-4 md:px-6 flex items-center justify-between shrink-0 z-30">
        <div className="flex items-center space-x-3">
          ...
          <span className="text-lg md:text-xl font-bold tracking-tight text-slate-800">
            EchoCore <span className="text-emerald-600 font-extrabold">AI</span>
          </span>
        </div>
          ...
          false ? (
            ...
            <div id="audio-output-player" className="bg-white border border-slate-200 rounded-2xl p-4 md:p-5 flex flex-col sm:flex-row items-stretch sm:items-center gap-4 sm:gap-6 shadow-sm transition-all duration-300 hover:scale-[1.01] hover:shadow-md">
              ...
              <div className="flex-1 min-w-0 space-y-2">
                ...
                <div 
                  className="relative w-full h-12 bg-slate-50/50 rounded-xl overflow-hidden border border-slate-100 flex items-center cursor-pointer"
                  onClick={(e) => {
                    const rect = e.currentTarget.getBoundingClientRect();
                    const clickX = e.clientX - rect.left;
                    const clickPercent = clickX / rect.width;
                    const targetTime = clickPercent * playDuration;
                    const limitStart = isTrimActive ? trimStart : 0;
                    const limitEnd = isTrimActive ? trimEnd : playDuration;
                    const clampedTime = Math.max(limitStart, Math.min(limitEnd, targetTime));
                    setPlayTime(Math.round(clampedTime));
                    showToast(`Seek to ${Math.round(clampedTime)}s`);
                  }}
                >
                  <canvas
                    ref={canvasRef}
            ...
            {/* NEW: Voice Emotion Analytics Panel */}
            <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 space-y-4">
              <div className="flex items-center space-x-2">
                <Activity className="w-4 h-4 text-indigo-600" />
                <h3 className="text-xs font-bold uppercase tracking-wider text-slate-700">Voice Emotion & Tone Analytics (ভয়েস আবেগ বিশ্লেষণ)</h3>
              </div>
              <p className="text-[11px] text-slate-500 leading-normal">
                Neural telemetry feedback based on active pitch, cadence speed, and character model selection.
              </p>
              
              <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 pt-1">
                {[
                  { name: "Joy / Vibrant", val: selectedEmotion === "Vibrant" ? 85 : (selectedEmotion === "Excited" ? 95 : (selectedEmotion === "Warm" ? 60 : 15)), color: "bg-amber-500", label: "Joy" },
                  { name: "Serenity / Calm", val: selectedEmotion === "Calm" ? 90 : (selectedEmotion === "Warm" ? 75 : (selectedEmotion === "Whisper" ? 65 : 20)), color: "bg-emerald-500", label: "Serenity" },
                  { name: "Sorrow / Pathos", val: selectedEmotion === "Sad" ? 85 : (selectedEmotion === "Whisper" ? 40 : 10), color: "bg-blue-500", label: "Sorrow" },
                  { name: "Anger / Passion", val: selectedEmotion === "Angry" ? 90 : (selectedEmotion === "Excited" ? 50 : 5), color: "bg-rose-500", label: "Anger" },
                  { name: "Mystery / Whisper", val: selectedEmotion === "Whisper" ? 95 : (selectedEmotion === "Calm" ? 40 : 12), color: "bg-purple-500", label: "Mystery" }
                ].map((emotion) => {
                  let adjustedVal = emotion.val;
                  if (emotion.label === "Joy" && pitchOffset > 0) adjustedVal = Math.min(100, adjustedVal + pitchOffset * 2);
                  if (emotion.label === "Serenity" && speechRate < 50) adjustedVal = Math.min(100, adjustedVal + (50 - speechRate) * 0.5);
                  if (emotion.label === "Anger" && speechRate > 55) adjustedVal = Math.min(100, adjustedVal + (speechRate - 55) * 0.4);
                  if (emotion.label === "Sorrow" && pitchOffset < 0) adjustedVal = Math.min(100, adjustedVal - pitchOffset * 2);
                  
                  adjustedVal = Math.max(5, Math.round(adjustedVal));
                  
                  return (
                    <div key={emotion.label} className="bg-white p-3 rounded-xl border border-slate-100 flex flex-col justify-between space-y-2">
                      <span className="text-[10px] font-bold text-slate-500 uppercase tracking-tight">{emotion.name}</span>
                      <div className="flex items-end space-x-2">
                        <span className="text-sm font-extrabold text-slate-800 font-mono">{adjustedVal}%</span>
                        <div className="flex-1 bg-slate-100 h-1.5 rounded-full overflow-hidden">
                          <div className={`h-full rounded-full ${emotion.color}`} style={{ width: `${adjustedVal}%` }} />
                        </div>
                      </div>
                    </div>
                  );
                  ...
                  <div className="flex flex-wrap gap-2 items-center">
                    ...
                    <div className="flex items-center space-x-1">
                      ...
                      <select
                        ...
                        className="bg-white border border-slate-200 rounded-lg px-2 py-1 text-xs text-slate-600 focus:outline-none focus:ring-1 focus:ring-blue-500 font-semibold cursor-pointer"
                      >
                        <option value="All">All Voices</option>
                        {Array.from(new Set(history.map(h => h.cloneName))).map(name => (
                    ...
                    {/* Favorites Toggle */}
                    <label className="flex items-center space-x-1.5 text-xs font-bold text-slate-600 cursor-pointer bg-white px-2.5 py-1 border border-slate-200 rounded-lg select-none">
                      <input
                        type="checkbox"
                      ...
                      />
                      <span className="text-[10px] uppercase font-bold tracking-tight">FAVORITES ONLY</span>
                    </label>
                  </div>
                  ...
                  <div className="flex gap-2 items-center">
                      ...
                      <input
                        ...
                        checked={
                          ...
                          history.filter(item => {
                            ...
                            const matchQuery = item.text.toLowerCase().includes(query) || item.cloneName.toLowerCase().includes(query);
                            const matchVoice = historyVoiceFilter === "All" || item.cloneName === historyVoiceFilter;
                            const matchFav = !showFavoritesOnly || item.favorite;
                            
                            let matchDate = true;
                            if (historyDateFilter === "Today") matchDate = item.date.includes("now") || item.date.includes("Today") || item.date.includes("minute");
                            else if (historyDateFilter === "Yesterday") matchDate = item.date.includes("Yesterday");
                            else if (historyDateFilter === "ThisWeek") matchDate = !item.date.includes("Month") && !item.date.includes("Year");
                            
                            return matchQuery && matchVoice && matchFav && matchDate;
                          }).every(item => selectedHistoryIds.includes(item.id))
                        }
                        onChange={() => {
                          ...
                          toggleSelectAllHistory(items);
                        }}
                        className="rounded border-slate-300 text-blue-600 focus:ring-blue-500 w-3.5 h-3.5 cursor-pointer"
                      />
                      <span className="text-[10px] uppercase font-bold tracking-tight">SELECT ALL</span>
                    </label>
                    ...
                    {selectedHistoryIds.length > 0 && (
                      <button
                        ...
                        className="px-2.5 py-1 text-[10px] font-bold text-red-600 bg-red-50 hover:bg-red-100 border border-red-200 rounded-lg transition-all flex items-center space-x-1"
                      >
                        <Trash2 className="w-3.5 h-3.5" />
                      ...
                      .map((hist) => {
                        ...
                        return (
                          <div
                            ...
                            className={`p-3.5 border rounded-xl cursor-pointer transition-all duration-200 flex items-start justify-between group ${
                              isSelected 
                          ...
                          >
                            <div className="flex items-start space-x-3 min-w-0 flex-1">
                              ...
                              <input
                                type="checkbox"
                                checked={isSelected}
                                onClick={(e) => toggleSelectHistory(hist.id, e)}
                                onChange={() => {}} // handled by onClick to avoid double triggers
                            ...
                            <div className="flex items-center space-x-2 shrink-0 mt-0.5">
                              ...
                              <button
                                onClick={(e) => toggleFavoriteHistory(hist.id, e)}
                                className={`p-1 rounded-lg transition-colors cursor-pointer ${
                                  hist.favorite 
                                    ? "text-rose-500 hover:text-rose-600" 
                                    : "text-slate-300 hover:text-slate-500 hover:bg-slate-50"
                                }`}
                                title={hist.favorite ? "Remove from favorites" : "Add to favorites"}
                              >
                              ...
                              <div className="p-1 rounded bg-slate-50 text-slate-300 group-hover:text-blue-500 transition-colors">
